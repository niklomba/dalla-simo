// Dalla Simo v1.17 — risoluzione prodotto da barcode: Open Facts universale + fallback
(function(){
'use strict';
function boot117ProductResolver(){
  ...
  async function fetchJson117(url,timeout=6500){
    ...
    try{
      ...
      return {found:true,nome,product:p,tipo:String(p.product_type||j.product_type||'').trim()};
    }finally{
      clearTimeout(t);
    }
  }
  ...
  async function risolviProdotto117(code){
    ...
    if(st.codiciProdotto[code]){
      return {
        ...
        tipo:st.codiciProdottoMeta?.[code]?.tipo||'',
        fonte:st.codiciProdottoMeta?.[code]?.fonte||'cache',
        stato:'cache'
      };
    }

    const fields='code,product_type,product_name_it,product_name,generic_name_it,generic_name,abbreviated_product_name_it,abbreviated_product_name,brands,quantity';
    const endpoints=[
      {fonte:'Open Facts universale',url:'https://world.openfoodfacts.org/api/v3/product/'+encodeURIComponent(code)+'?product_type=all&lc=it&cc=it&fields='+fields},
      {fonte:'Open Food Facts',url:'https://world.openfoodfacts.org/api/v2/product/'+encodeURIComponent(code)+'.json?lc=it&cc=it&fields='+fields},
      {fonte:'Open Products Facts',url:'https://world.openproductsfacts.org/api/v2/product/'+encodeURIComponent(code)+'.json?lc=it&cc=it&fields='+fields}
    ];
    ...
    for(const ep of endpoints){
      try{
        ...
        if(x.found){
          ...
          st.codiciProdottoMeta[code]={tipo:x.tipo||'',fonte:ep.fonte,aggiornatoIl:new Date().toISOString()};
          save();
          return {nome:x.nome,tipo:x.tipo||'',fonte:ep.fonte,stato:'trovato'};
        }
      }catch(e){
        ultimoErrore=e;
  ...
  window.gestisciCodice116=async function(code,mode){
    ...
    if(!nome){
      const testo=info.stato==='errore_rete'
        ...
        :'Prodotto non presente nei database Open Facts. Scrivi il nome da associare a questo codice:';
      const manuale=prompt(testo,'');
      if(!manuale||!manuale.trim())return;
      nome=manuale.trim();
      st.codiciProdotto[code]=nome;
      st.codiciProdottoMeta[code]={tipo:'manuale',fonte:'inserimento manuale',aggiornatoIl:new Date().toISOString()};
      save();
    }
    ...
    const nonFood=info.tipo&&info.tipo!=='food'&&info.tipo!=='manuale';
    if(nonFood&&!confirm('Ho riconosciuto “'+nome+'” come prodotto non alimentare ('+info.tipo+'). Vuoi aggiungerlo comunque alla Dispensa?')){
      return;
    }
  ...
  window.__DALLA_SIMO_PRODUCT_LOOKUP__='v117-universal-open-facts';
}
